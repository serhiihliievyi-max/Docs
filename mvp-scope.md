...
| KYC верификация | Только загрузка файла. Флаг `is_verified` проставляется вручную | v2 |
| Forgot/reset password | Некритично для демо | v2 |
| Отзывы на пользователя | Достаточно отзывов на машину | v2 |
| `DELETE /cars/:id` | Достаточно `PATCH` с `status: inactive` | — |
...
### Верификация
- Поле `is_verified: boolean` в таблице users
- Проставляется вручную через Supabase Studio
- Никакой автоматической проверки документов
